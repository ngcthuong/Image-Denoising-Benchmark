function imr_final = NLH_CC_Fast(im_n)
...
for ll = 1:k 
        ...
        [imr1,imr2,imr3] = NLH_Color_Fast(N1,N2,N3,Ns,N_step,double(alpha*imr1+(1-alpha)*imp1),double(alpha*imr2+(1-alpha)*imp2),double(alpha*imr3+(1-alpha)*imp3),single(Thr),...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       [y_est1,y_est2,y_est3] = NLH_Color_Wiener_Fast(N1,N2,N3,Ns,N_step1,double(imp1),double(imp2),double(imp3),single(gamma),...
                sigma_est_b/255, double(dis_map),double(imr1*gamma),double(imr2*gamma),double(imr3*gamma),double(imr2*beta+imp1*(1-beta)));            

                        
        [y_est1,y_est2,y_est3] = NLH_Color_Wiener_Fast(N1,N2,N3,Ns,N_step2,double(y_est1),double(y_est2),double(y_est3),single(gamma),...
